...
프로젝트는 기능별로 라우터를 분리하여 각 기능의 책임을 구분했습니다.

* `routers/data.py`: 관광 데이터의 등록, 조회, 수정, 삭제 및 데이터 요약을 담당합니다.
* `routers/chat.py`: 사용자 질문을 처리하고 관광 데이터 요약 정보를 AI에 전달하여 답변을 생성합니다.
* `routers/conversations.py`: AI 질문과 답변의 저장, 목록 조회, 개별 대화 조회 및 삭제를 담당합니다.
* `schemas/data.py`: 관광 데이터 입력값의 형식과 범위를 검증합니다.
* `main.py`: FastAPI 앱을 생성하고 각 라우터를 등록하며 프론트엔드 정적 파일을 제공합니다.

현재 프로젝트 규모에서는 별도의 서비스 계층을 두기보다 기능별 라우터와 함수 단위로 책임을 분리했습니다. 향후 기능이 확대될 경우 데이터 처리 및 AI 관련 비즈니스 로직을 별도의 서비스 계층으로 분리할 수 있도록 구성했습니다.
...
관광 데이터 요약은 `routers/chat.py`의 `get_data_summary()` 함수에서 별도로 처리합니다.

요약 로직을 별도 함수로 분리한 이유는 AI 질문 처리와 데이터 집계 로직의 책임을 구분하고, 동일한 요약 정보를 다른 기능에서도 재사용할 수 있도록 하기 위해서입니다.

현재는 AI 채팅에서 요약 데이터를 활용하고 있으며, 향후 대시보드나 관광 데이터 분석 기능에서도 동일한 요약 로직을 재사용할 수 있습니다.

요약 기준을 변경할 경우 `routers/data.py`의 `/summary` API와 `routers/chat.py`의 `get_data_summary()`를 확인하여 수정할 수 있습니다.

### 요약 성능 및 갱신 고려사항

현재는 AI 질문 처리 시 필요한 관광 데이터 요약을 생성하므로 데이터 규모가 커질수록 Firestore 조회와 집계에 따른 처리 시간이 증가할 수 있습니다.

현재 프로젝트 규모에서는 질문 시 최신 데이터를 즉시 반영하는 방식이 적절하다고 판단했습니다. 향후 데이터가 증가할 경우에는 요약 결과를 일정 주기로 갱신하거나 캐싱하여 반복적인 Firestore 조회와 집계 비용을 줄일 수 있습니다.

캐싱을 적용할 경우 새로운 데이터 등록·수정·삭제 시 캐시를 무효화하거나 정해진 주기에 따라 요약을 다시 생성하여 데이터 최신성을 유지할 수 있습니다.
...
### 시스템 프롬프트에 데이터 요약 정보를 주입하는 이유와 고려사항

관광 데이터 전체를 매번 AI에 전달하는 대신 요약 정보를 시스템 프롬프트에 포함하여 답변에 필요한 핵심 데이터를 제공했습니다.

장점:

* 전체 데이터를 직접 전달하는 것보다 프롬프트 크기를 줄일 수 있습니다.
* AI가 관광 데이터의 핵심 수치와 추세를 기준으로 답변하도록 유도할 수 있습니다.
* 데이터 구조가 변경되어도 요약 결과를 중심으로 AI 처리 흐름을 유지할 수 있습니다.

고려사항:

* 데이터가 증가하면 요약 정보 자체의 크기도 커질 수 있어 프롬프트 관리가 필요합니다.
* 요약 기준이 변경되면 AI 답변 결과에도 영향을 줄 수 있습니다.
* 장기적으로는 요약 데이터의 버전 관리와 최대 크기 제한을 적용하는 것이 좋습니다.
...
FastAPI 서버는 Render를 이용하여 배포했습니다.
...
FastAPI의 Swagger UI를 통해 주요 API 엔드포인트를 확인하고 테스트했습니다.
...
Swagger UI에서 데이터 CRUD, 관광 데이터 요약, AI 채팅, 대화 목록 및 조회 API의 엔드포인트를 확인할 수 있습니다.

![Swagger 실행 화면](images/05_Swagger_실행화면.png)


### 배포 실행 확인

배포 후 실제 Render 서버에 접속하여 정상 응답을 확인했습니다.
...
실제 배포 환경에서 데이터 조회, AI 질문, 데이터 등록·수정·삭제, 대화 저장 및 조회 기능을 테스트하여 정상 동작을 확인했습니다.

![배포 실제 화면](images/04_배포_실제화면.png)


### Render 콜드스타트 안내

Render 무료 환경에서는 일정 시간 요청이 없을 경우 서비스가 일시적으로 중지될 수 있습니다. 이 경우 다시 요청했을 때 서버가 재시작되면서 첫 접속이나 첫 API 요청의 응답이 평소보다 늦어질 수 있습니다.

따라서 장시간 사용하지 않은 후 처음 접속할 경우 초기 로딩 시간이 발생할 수 있으며, 이는 무료 호스팅 환경의 특성에 따른 것입니다. 실제 운영 환경에서는 상시 실행되는 유료 인스턴스나 프리워밍 방식을 적용할 수 있습니다.


### Frontend

HTML/CSS/JavaScript 기반 프론트엔드는 FastAPI 서버를 통해 함께 실행되도록 구성했습니다.
...
별도의 프론트엔드 서버를 사용하지 않고 FastAPI가 정적 파일을 제공하며, 프론트엔드에서 실제 Render 배포 주소를 Backend API 주소로 사용하도록 구성했습니다.
